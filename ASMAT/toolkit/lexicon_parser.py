...
def save_lexicon_as_dataset(lexicon, path, sep='\t'):
    data = [[str(score), word] for word, score in lexicon.items()]
...
def argparser():
    ...
    parser.add_argument('-skip_first', type=bool, default=False, help="skip_first")
    parser.add_argument('-save_dataset', type=str, help="save in the ASMAT dataset format")
    return parser
...
if __name__ == "__main__":  
    ...
    if args.save_dataset is not None:
        l = read_lex(path_in)
        save_lexicon_as_dataset(l, args.save_dataset)
    elif args.type == 'mpqa':
